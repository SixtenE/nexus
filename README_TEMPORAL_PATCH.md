...
1) Install deps:
   ```bash
   uv pip install -r backend/requirements_temporal.txt
   ```
   or
   ```bash
   pip install -r backend/requirements_temporal.txt
   ```
...
4) Start API (standalone minimal app if you don't want to touch your existing app):
   ```bash
   python backend/main_temporal.py
...
5) Frontend route:
   Add a route to render `TemporalState` page (see frontend/src/pages/README_TEMPORAL.md).
